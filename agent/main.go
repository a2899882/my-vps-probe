...
var globalCountryCode = "OT"

func init() {
go func() {
resp, err := http.Get("http://ip-api.com/json/")
if err == nil { defer resp.Body.Close(); var res struct { CountryCode string `json:"countryCode"` }; json.NewDecoder(resp.Body).Decode(&res); if res.CountryCode != "" { globalCountryCode = res.CountryCode } }
}()
}
...
// 【核心修复】：智能协议回退机制 (无惧 Cloudflare 拦截)
conn, _, err := websocket.DefaultDialer.Dial(wsURL, nil)
if err != nil {
log.Printf("首选协议 %s 连接失败, 尝试智能切换协议穿透防火墙...\n", wsScheme)
if wsScheme == "ws://" { wsScheme = "wss://" } else { wsScheme = "ws://" }
wsURL = fmt.Sprintf("%s%s/ws?token=%s", wsScheme, cleanAddr, token)
conn, _, err = websocket.DefaultDialer.Dial(wsURL, nil)
if err != nil { log.Println("彻底连接失败, 请检查网络:", err); return }
}
defer conn.Close()

var instruction AgentInstruction
if err := conn.ReadJSON(&instruction); err != nil { return }
log.Printf("✅ 成功连接主控！我是 [%s]，自动识别地区代码: [%s]\n", instruction.ServerName, globalCountryCode)
...
var status ServerStatus; status.CountryCode = globalCountryCode
...
if len(nInfo) > 0 { status.NetInTransfer = nInfo[0].BytesRecv; status.NetOutTransfer = nInfo[0].BytesSent; if lastNetBytesRecv > 0 { status.NetInSpeed = (nInfo[0].BytesRecv - lastNetBytesRecv) / 2; status.NetOutSpeed = (nInfo[0].BytesSent - lastNetBytesSent) / 2 }; lastNetBytesRecv = nInfo[0].BytesRecv; lastNetBytesSent = nInfo[0].BytesSent }
tickCount++; isMinuteTick := (tickCount % 30 == 0); var pingResults []PingResult
for _, task := range tasks {
delay, success := doPing(task.Host); t, ok := trackers[task.Name]; if !ok { t = &PingTracker{History: make([]int, 0)}; trackers[task.Name] = t }
t.LastDelay = delay; if !success { t.FailsThisMin++ }; if isMinuteTick { if t.FailsThisMin > 0 { t.History = append(t.History, 0) } else { t.History = append(t.History, 1) }; if len(t.History) > 60 { t.History = t.History[1:] }; t.FailsThisMin = 0 }
failCount := 0; for _, v := range t.History { if v == 0 { failCount++ } }; lossRate := 0.0; if len(t.History) > 0 { lossRate = float64(failCount) / float64(len(t.History)) * 100 }
pingResults = append(pingResults, PingResult{ TargetName: task.Name, CurrentDelay: delay, LossRate: lossRate, History: t.History })
}
status.PingStatuses = pingResults; return status
}

func doPing(ip string) (float64, bool) {
cmd := exec.Command("ping", "-c", "1", "-W", "1", ip); out, err := cmd.Output(); if err != nil { return 0, false }
idx := strings.Index(string(out), "time="); if idx != -1 { endIdx := strings.Index(string(out)[idx:], " ms"); if endIdx != -1 { var delay float64; fmt.Sscanf(string(out)[idx+5:idx+endIdx], "%f", &delay); return delay, true } }; return 0, true
}
