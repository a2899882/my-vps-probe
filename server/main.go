...
go func() {
for { time.Sleep(1 * time.Minute); saveHistoryToDB(); db.Exec("DELETE FROM ping_history WHERE timestamp <= datetime('now', '-3 days')") }
}()
}
func saveHistoryToDB() {
mapMutex.RLock(); defer mapMutex.RUnlock(); tx, _ := db.Begin(); defer tx.Commit()
stmt, _ := tx.Prepare("INSERT INTO ping_history (server_id, target_name, delay, loss_rate) VALUES (?, ?, ?, ?)"); defer stmt.Close()
...
appConfig = common.AppConfig{ Nodes: []common.NodeConfig{ {ID: "node-1", Name: "主控测试机", Token: "my_secret_token_123", ExpireDate: "2027/05/13"} }, PingTasks: []common.PingTask{ {Name: "Cloudflare", Host: "1.1.1.1"}, {Name: "Google", Host: "8.8.8.8"} } }
...
http.HandleFunc("/download/common/model.go", func(w http.ResponseWriter, r *http.Request) { http.ServeFile(w, r, "common/model.go") })

http.HandleFunc("/api/admin/config", basicAuth(func(w http.ResponseWriter, r *http.Request) {
w.Header().Set("Content-Type", "application/json")
if r.Method == "GET" { configMutex.RLock(); json.NewEncoder(w).Encode(appConfig); configMutex.RUnlock() } else if r.Method == "POST" {
var newConfig common.AppConfig
if err := json.NewDecoder(r.Body).Decode(&newConfig); err == nil {
configMutex.Lock(); appConfig = newConfig; data, _ := json.MarshalIndent(appConfig, "", "  "); os.WriteFile("config.json", data, 0644); configMutex.Unlock(); w.Write([]byte(`{"status":"ok"}`))
}
}
}))
...
for {
if err := conn.ReadJSON(&st); err != nil { mapMutex.Lock(); st = serverStatusMap[cNode.ID]; st.IsOnline = false; serverStatusMap[cNode.ID] = st; mapMutex.Unlock(); break }
st.ServerID = cNode.ID; st.IsOnline = true; mapMutex.Lock(); serverStatusMap[cNode.ID] = st; mapMutex.Unlock()
}
})
http.HandleFunc("/api/status", func(w http.ResponseWriter, r *http.Request) {
w.Header().Set("Content-Type", "application/json"); configMutex.RLock(); mapMutex.RLock(); var result []FrontendNode
...
w.Header().Set("Content-Type", "application/json")
serverID := r.URL.Query().Get("server_id"); hours := r.URL.Query().Get("hours"); if hours == "" { hours = "24" }
...
fmt.Println("🚀 服务端启动！已开启一键安装脚本及组件分发接口！")
